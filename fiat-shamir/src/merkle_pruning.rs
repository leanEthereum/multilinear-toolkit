    ...
    pub fn prune(self) -> PrunedMerklePaths<Data, F> {
        ...
        let paths = deduped
            ...
            .map(|(i, path)| {
                ...
                let levels = i.checked_sub(1).map_or(merkle_height, |j| {
                    lca_level(deduped[j].leaf_index, leaf_idx)
                });
                let skip = deduped
                    .get(i + 1)
                    .map(|p| lca_level(leaf_idx, p.leaf_index) - 1);
...
impl<Data: Clone, F: Clone> PrunedMerklePaths<Data, F> {
    pub fn restore(
        ...
        let skip = |i: usize| {
            self.paths
                .get(i + 1)
                .map(|p| lca_level(self.paths[i].0, p.0) - 1)
        };
...
mod tests {
    ...
    fn hash_combine(
        left: &[u8; DIGEST_LEN_FE],
        right: &[u8; DIGEST_LEN_FE],
    ) -> [u8; DIGEST_LEN_FE] {
    ...
    fn test_duplicated_paths_preserved() {
        ...
        for (i, (orig, rest)) in paths.0.iter().zip(restored.0.iter()).enumerate() {
            assert_eq!(
                orig.leaf_index, rest.leaf_index,
                "Path {} leaf_index mismatch",
                i
            );
            assert_eq!(
                orig.leaf_data, rest.leaf_data,
                "Path {} leaf_data mismatch",
                i
            );
            assert_eq!(
